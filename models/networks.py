...
def define_net(input_nc, output_nc, net_name, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    """
    ...
    if norm == 'batch':
        batch_norm = True
    if net_name == 'TWBAG':
        net = TWBAG(input_nc, output_nc)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % net_inpaint)
